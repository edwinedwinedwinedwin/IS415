(function( $ ) {
    ...
    if (typeof(Shiny) != "undefined") {
      (function() {
        ...
        function onLicense(license) {
          ...
          var message = 'Your RStudio Connect ' + noun + ' expired';
        ...
        var sockjsUrl = loc + "/__sockjs__/n=" + robustId + "/t=" + token + '/w=' + worker  + '/s=' + (isSubApp()? '1':'0');
  ...
  MultiplexClient.prototype.scheduleReconnect_p = function(time, count, expires) {
    ...
    if (count < 0){
      ...
      if (count < 10){
        ...
        interval = Math.min(timeout * 1000, interval); // Max delay.
      } else { 
  ...
  MultiplexClient.prototype.startReconnect = function(){
    ...
    if (!self._disconnected){
      ...
      var time = Date.now();
      var timeout = 0;
      self.scheduleReconnect_p(time, -1, time + timeout * 1000)
      .fail(function(){
